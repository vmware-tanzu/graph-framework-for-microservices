...
func (p *Package) GetHttpCodesResponses() map[string]HTTPCodesResponse {
			...
			if valueSpec, ok := spec.(*ast.ValueSpec); ok {
				...
				if value, ok := valueSpec.Values[0].(*ast.CompositeLit); ok {
					...
					for _, elt := range value.Elts {
						var responseKey ResponseCode

						kv := elt.(*ast.KeyValueExpr)
						// Extract correct key
						switch key := kv.Key.(type) {
						case *ast.SelectorExpr:
							responseKey = ResponseCode(httpStatusCodes[key.Sel.String()])
						case *ast.Ident:
							if key.Name == "DefaultHTTPErrorCode" {
								responseKey = DefaultHTTPErrorCode
							}
						}
