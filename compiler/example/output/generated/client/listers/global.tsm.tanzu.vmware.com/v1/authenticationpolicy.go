...
type AuthenticationPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AuthenticationPolicy, err error)
	// Get retrieves the AuthenticationPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AuthenticationPolicy, error)
...
// List lists all AuthenticationPolicies in the indexer.
func (s *authenticationPolicyLister) List(selector labels.Selector) (ret []*v1.AuthenticationPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AuthenticationPolicy))
	})
	return ret, err
}

// Get retrieves the AuthenticationPolicy from the index for a given name.
func (s *authenticationPolicyLister) Get(name string) (*v1.AuthenticationPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("authenticationpolicy"), name)
	}
	return obj.(*v1.AuthenticationPolicy), nil
}
