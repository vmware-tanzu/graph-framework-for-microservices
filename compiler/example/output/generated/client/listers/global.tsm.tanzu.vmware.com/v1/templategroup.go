...
type TemplateGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v1.TemplateGroup, err error)
	// Get retrieves the TemplateGroup from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.TemplateGroup, error)
...
// List lists all TemplateGroups in the indexer.
func (s *templateGroupLister) List(selector labels.Selector) (ret []*v1.TemplateGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.TemplateGroup))
	})
	return ret, err
}

// Get retrieves the TemplateGroup from the index for a given name.
func (s *templateGroupLister) Get(name string) (*v1.TemplateGroup, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("templategroup"), name)
	}
	return obj.(*v1.TemplateGroup), nil
}
