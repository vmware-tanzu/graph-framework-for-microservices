...
type EnvoyFilterLister interface {
	...
	List(selector labels.Selector) (ret []*v1.EnvoyFilter, err error)
	// Get retrieves the EnvoyFilter from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.EnvoyFilter, error)
...
// List lists all EnvoyFilters in the indexer.
func (s *envoyFilterLister) List(selector labels.Selector) (ret []*v1.EnvoyFilter, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.EnvoyFilter))
	})
	return ret, err
}

// Get retrieves the EnvoyFilter from the index for a given name.
func (s *envoyFilterLister) Get(name string) (*v1.EnvoyFilter, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("envoyfilter"), name)
	}
	return obj.(*v1.EnvoyFilter), nil
}
