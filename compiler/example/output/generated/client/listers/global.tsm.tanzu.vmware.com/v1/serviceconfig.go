...
type ServiceConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ServiceConfig, err error)
	// Get retrieves the ServiceConfig from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ServiceConfig, error)
...
// List lists all ServiceConfigs in the indexer.
func (s *serviceConfigLister) List(selector labels.Selector) (ret []*v1.ServiceConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServiceConfig))
	})
	return ret, err
}

// Get retrieves the ServiceConfig from the index for a given name.
func (s *serviceConfigLister) Get(name string) (*v1.ServiceConfig, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("serviceconfig"), name)
	}
	return obj.(*v1.ServiceConfig), nil
}
