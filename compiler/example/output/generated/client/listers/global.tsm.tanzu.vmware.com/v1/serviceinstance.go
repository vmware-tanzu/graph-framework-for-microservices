...
type ServiceInstanceLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ServiceInstance, err error)
	// Get retrieves the ServiceInstance from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ServiceInstance, error)
...
// List lists all ServiceInstances in the indexer.
func (s *serviceInstanceLister) List(selector labels.Selector) (ret []*v1.ServiceInstance, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServiceInstance))
	})
	return ret, err
}

// Get retrieves the ServiceInstance from the index for a given name.
func (s *serviceInstanceLister) Get(name string) (*v1.ServiceInstance, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("serviceinstance"), name)
	}
	return obj.(*v1.ServiceInstance), nil
}
