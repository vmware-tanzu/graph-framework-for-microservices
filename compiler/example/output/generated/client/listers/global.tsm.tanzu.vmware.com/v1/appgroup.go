...
type AppGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AppGroup, err error)
	// Get retrieves the AppGroup from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AppGroup, error)
...
// List lists all AppGroups in the indexer.
func (s *appGroupLister) List(selector labels.Selector) (ret []*v1.AppGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AppGroup))
	})
	return ret, err
}

// Get retrieves the AppGroup from the index for a given name.
func (s *appGroupLister) Get(name string) (*v1.AppGroup, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("appgroup"), name)
	}
	return obj.(*v1.AppGroup), nil
}
