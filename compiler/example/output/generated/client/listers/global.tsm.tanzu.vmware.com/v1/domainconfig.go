...
type DomainConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1.DomainConfig, err error)
	// Get retrieves the DomainConfig from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.DomainConfig, error)
...
// List lists all DomainConfigs in the indexer.
func (s *domainConfigLister) List(selector labels.Selector) (ret []*v1.DomainConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.DomainConfig))
	})
	return ret, err
}

// Get retrieves the DomainConfig from the index for a given name.
func (s *domainConfigLister) Get(name string) (*v1.DomainConfig, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("domainconfig"), name)
	}
	return obj.(*v1.DomainConfig), nil
}
