...
type UserGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v1.UserGroup, err error)
	// Get retrieves the UserGroup from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.UserGroup, error)
...
// List lists all UserGroups in the indexer.
func (s *userGroupLister) List(selector labels.Selector) (ret []*v1.UserGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.UserGroup))
	})
	return ret, err
}

// Get retrieves the UserGroup from the index for a given name.
func (s *userGroupLister) Get(name string) (*v1.UserGroup, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("usergroup"), name)
	}
	return obj.(*v1.UserGroup), nil
}
